...
from pm4py.algo.discovery.heuristics import algorithm as miner
...
def heuristic_miner(event_log: str, bpmn_out: str) -> ModelMetrics:
    log = xes_importer.apply(event_log)
    net, im, fm = miner.apply(log, parameters={
        miner.Variants.CLASSIC.value.Parameters.DEPENDENCY_THRESH: 0.99})
